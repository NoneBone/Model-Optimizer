...
如果输入模型是图像分类类型，请使用以下脚本对其进行评估。该脚本会自动下载并使用…… [ILSVRC/imagenet-1k](https://huggingface.co/datasets/ILSVRC/imagenet-1k) 数据集来自 Hugging Face。此受限存储库需要通过 Hugging Face 访问令牌进行身份验证。详情请参阅 <https://huggingface.co/docs/hub/en/security-tokens>。
